...
class KnowledgeWriteTool(private val manager: KnowledgeManager) : AgentTool {
    ...
    override val description =
        "新增或更新一条服务器知识库条目（FAQ / 运营记录 / 规则约定等）。仅写确有长期复用价值的内容，别记一次性琐碎。传已存在的 id 则覆盖更新。"
    override val inputSchema = """{"type":"object","properties":{"id":{"type":"string","description":"条目 id；留空=新建（按标题生成），填已存在 id=覆盖更新"},"title":{"type":"string","description":"标题"},"content":{"type":"string","description":"正文（Markdown）"},"tags":{"type":"array","items":{"type":"string"},"description":"可选标签"}},"required":["title","content"]}"""
    ...
    override fun execute(toolCallId: String, inputJson: String): ToolResult = runCatching {
        ...
        val e = manager.save(node["id"]?.asText()?.takeIf { it.isNotBlank() }, title, content, tags)
