...
/**
 * 让中心 Agent 在子服上**读/改/删文件**（改配置、装插件的落地手），经 [MessageBus] 下发到子服执行。
...
/** 读取子服上的文件内容。 */
class RemoteReadFileTool(private val bus: MessageBus, private val timeoutMs: Long, private val audit: AuditLog) : AgentTool {
...
/** 列出子服上某目录的文件。 */
class RemoteListDirTool(private val bus: MessageBus, private val timeoutMs: Long, private val audit: AuditLog) : AgentTool {
    private val mapper = ObjectMapper()
...
class RemoteWriteFileTool(private val bus: MessageBus, private val timeoutMs: Long, private val audit: AuditLog) : AgentTool {
    private val mapper = ObjectMapper()
