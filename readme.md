...
- 🧩 **CMI 插件集成** — 经济/传送/封禁/礼包/用户查询，结构化 API 调用（非命令拼接），装了 CMI 自动生效
- 🧪 **技能扩展（Skill）** — 服主放 `SKILL.md` 给 Agent 加能力，免重编译；纯文字流程 / Groovy 脚本 / **多阶段工作流**三类，网页可视化增删改 + AI 起草 + 测试运行
...
- **技能即扩展** — 对齐 Anthropic Agent Skills：`SKILL.md`（`name`/`description` + 正文）按需披露，脚本可选。脚本走内嵌 GroovyShell、主线程跳转 + 超时看门狗。工作流技能支持多阶段编排（条件跳转 / 循环 / 变量传递 / 失败重试 / skill 间调用），与命令/MCP/远端工具共用同一 `AgentTool` 抽象，核心零改动即热插拔。

## 技能扩展（Skill）

让服主**不改源码、不重编译**就给 Agent 加能力——把技能丢进 `skills/` 目录即生效（热重载）。对齐 Anthropic Agent Skills 的形态，三种类型：
...
| 🔗 **工作流** | `SKILL.md` 声明多步骤，按序执行工具/脚本/其他技能 | 引擎自动编排，支持条件跳转、循环、变量传递、失败重试 | 出生点搭建、世界备份、新手引导等多步操作 |
...
---
name: setup_spawn
description: 设置服务器出生点区域（清障+铺设+设重生点+公告）
tags: [运维, 建筑]
permission: admin
params:
  world:
...
steps:
  - id: clear
    ...
    args:
      command: "/fill 0 63 0 {radius} 80 {radius} air"
  - id: floor
    name: 铺设地面
    tool: run_command_on_server
    args:
      command: "/fill 0 62 0 {radius} 62 {radius} {block}"
  - id: announce
    name: 全服公告
    script: |
      return "出生点已设置：世界={params.world}，半径={params.radius}"
---
当腐竹要求「设置出生点」时调用，引擎按顺序执行四步。
```

工作流步骤支持：
- **变量插值** — `{var}` / `{params.xxx}` 跨步骤传数据
- **条件跳转** — `condition: "expr"` Groovy 表达式控制是否执行
- **循环** — `repeat` / `forEach` 对集合迭代执行
- **失败策略** — `onFail: abort|continue|retry:3`
- **skill 间调用** — `skill: other_name` 递归执行另一个技能
- **参数约束** — `optional` / `default=value` / `enum=[a,b,c]`

- **安全** — 能往 `skills/` 放文件的即服主，技能视为「已审过的确定性扩展」：不过命令护栏、无人值守也可调，但每次执行记 `audit`；脚本在主线程跑、带超时看门狗。
- **对话式创建** — 服主说人话（"帮我做个技能，每天给在线玩家发钻石"），AI 自动生成 SKILL.md + Groovy 脚本并落盘，保存即生效。不需要会写任何代码。
...
- [x] 技能扩展（Skill：纯文字 / Groovy 脚本 / 多阶段工作流，网页管理 + AI 起草 + 跨服调用）
- [ ] embedding 语义检索（接口已留）
- [ ] 商品定价（待真实成交数据校准）
- [ ] 真实服务器长期压测
