...
class ScheduleTool(
    ...
    override val inputSchema = """{"type":"object","properties":{"operation":{"type":"string","description":"操作类型","enum":["create","list","delete","toggle"]},"name":{"type":"string","description":"任务名称（create 时必填）"},"action":{"type":"string","description":"动作类型：skill（调技能）/ broadcast（广播）/ command（执行命令）/ tools（AI实时决策）","enum":["skill","broadcast","command","tools"]},"target":{"type":"string","description":"目标子服名（skill/command/broadcast 用；* 或留空=全部在线子服）"},"skill":{"type":"string","description":"技能名（action=skill 时必填，须已存在）"},"payload":{"type":"string","description":"动作内容：action=broadcast 时为广播文案，action=command 时为命令，action=tools 时为 AI 指令"},"schedule":{"type":"string","description":"调度方式，如 'daily 20:00'（每天20点）、'daily 08:00 1,2,3,4,5'（工作日8点）、'interval 30'（每30分钟）"},"id":{"type":"string","description":"任务 ID（delete/toggle 时必填）"}},"required":["operation"]}"""
    ...
    private fun doCreate(toolCallId: String, node: com.fasterxml.jackson.databind.JsonNode): ToolResult {
        ...
        val payload = when (action) {
            ...
            "tools" -> node["payload"]?.asText()?.takeIf { it.isNotBlank() }
                ?: return ToolResult.error(toolCallId, "action=tools 时需要指定 AI 指令（payload）")
            else -> return ToolResult.error(toolCallId, "未知动作类型：$action（支持 skill/broadcast/command/tools）")
        }
