...
class WorkflowEngine(
    ...
    fun execute(def: SkillDef, params: Map<String, Any?>, depth: Int = 0): WorkflowResult {
        ...
        for (group in groups) {
            if (group.size == 1 && !group[0].parallel) {
                ...
                val assign = step.assign
                if (assign != null && result.value != null) {
                    ctx[assign] = result.value
                }
            } else {
                ...
                for (step in group) {
                    ...
                    val assign = step.assign
                    if (assign != null && stepResult?.value != null) {
                        ctx[assign] = stepResult.value
                    }
                }
            }
        }
    ...
    private fun resolveLoop(step: WorkflowStep, ctx: Map<String, Any?>): List<Any?>? {
        ...
        if (repeat != null) {
            val result = evalExpression(repeat, ctx)
            return when (result) {
